// IMPORTANT: This URL must be set correctly for deployment.
// If your backend is proxied (e.g., by Netlify/Vercel or in the Canvas environment), an empty string might work.
// Otherwise, replace with your full backend endpoint (e.g., "https://api.myapp.com").
const backendUrl = "";
...
// NOTE: These relative paths (e.g., "/burgerking1.png") must exist in your deployed assets folder.
const brandLogos = {
...
async function fetchBranches(countryId) {
  try {
    ...
    const res = await fetch(`${backendUrl}/branches?country_id=${countryId}`);
    ...
    branches.forEach(branch => {
      ...
      // Placeholder SVG for unknown brands
      ...
      branchItem.innerHTML = `
        <img src="${logoUrl}" 
             ...
             onerror="this.src='data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAyNDAgMjQwIiBmaWxsPSIjMDAwIj48dGV4dCB4PSI4MCIgeT0iMTMwIiBmb250LWZhbWlseT0iQXJpYWwiIGZvbnQtc2l6ZT0iMTIiPkJyYW5jaDwvdGV4dD48L2ZzZz4='">
...
async function fetchItems(countryId, branchId) {
  try {
    ...
    const res = await fetch(`${backendUrl}/items?country_id=${countryId}&branch_id=${branchId}`);
...
function displayResults(items) {
  ...
  // Food icons for different categories (Font Awesome assumed)
  const foodIcons = [
...
randomBtn.addEventListener("click", () => {
  if (foodItems.length > 0) {
    ...
    // Replaced alert() with UI feedback in the results div
    resultsDiv.innerHTML = `
      <div class="col-span-full text-center py-4 text-red-600 bg-red-50 rounded-lg p-4">
        <i class="fas fa-exclamation-circle text-2xl mb-2"></i>
        <p class="font-medium">Please select a country and branch first to load menu items.</p>
      </div>
    `;
    console.error("Attempted random selection with no items loaded.");
  }
});
...
clearFilters.addEventListener("click", () => {
  ...
  if (foodItems.length > 0) {
    displayResults(foodItems);
     ...
     // Re-fetch countries if everything was cleared and no data is present
     fetchCountries();
...
countrySelect.addEventListener("change", () => {
  ...
  // Reset branch selection when country changes
  currentBranchId = null;
  selectedBranch.textContent = "Select a Branch";
  ...
  if (currentCountryId) {
    ...
    // Hide search section until branch is selected
    searchSection.classList.add("hidden");
...
// Add interactivity to brand cards
document.addEventListener('DOMContentLoaded', function() {
  ...
  brandCards.forEach(card => {
    card.addEventListener('click', function() {
      ...
      // Replaced alert() with a console log for the dummy functionality
      console.info(`Simulating click for ${brand}. This would trigger country/branch selection.`);
      // In a real application, you would auto-select the country/branch based on this brand click
    });
  });
});
